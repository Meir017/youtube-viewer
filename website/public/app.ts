...
// ─────────────────────────────────────────────────────────────────
const UI_STATE_STORAGE_KEY = 'youtube-viewer:ui-state:v1';
let uiStateLoaded = false; // Set true after restoreUiState; gates saves during boot.
...
function snapshotUiState() {
    let view = 'videos';
    if (showHighlights) view = 'highlights';
    else if (showStarredVideos) view = 'starred';
    else if (showHiddenVideos) view = 'hidden';
    return {
        v: 1,
...
// Apply saved scalar/filter state into module-level vars and DOM inputs
// (collection-scoped state — channel index validation — happens after
// the collection's channels are loaded).
function applyUiStateScalars(saved) {
    ...
    switch (saved.view) {
        case 'highlights': showHighlights = true; showHiddenVideos = false; showStarredVideos = false; break;
        case 'starred': showStarredVideos = true; showHiddenVideos = false; showHighlights = false; break;
        case 'hidden': showHiddenVideos = true; showStarredVideos = false; showHighlights = false; break;
        default: showHighlights = false; showHiddenVideos = false; showStarredVideos = false;
    }

    // Reflect into DOM inputs (these elements are static in index.html)
    ...
    // Sort buttons — find the matching button and mark it active, set its order/icon.
    ...
    // Age preset buttons: clear unless minDays === 0 and maxDays matches a preset.
...
async function loadCollections() {
    ...
    try {
        ...
        if (chosenId) {
            activeCollectionId = chosenId;
            await loadCollectionChannels(chosenId);
        ...
        // Mark cache hot so subsequent mutations get persisted; then save
        // once to normalise the stored payload (e.g. dropped collectionId).
        uiStateLoaded = true;
        saveUiState();
...
function renderVideoCard(video) {
    ...
    return `
        <article class="video-card${publishDate ? ' is-enriched' : ''}${isStarred ? ' is-starred' : ''}" data-video-id="${videoId}" onmouseenter="handleCardHover(event)" onmouseleave="handleCardLeave(event)" onclick="openVideoModalById('${videoId}')" style="cursor: pointer;">
...
// After ~350ms of hovering a video card on a fine-pointer device, we show a
// centered modal-style preview containing a large thumbnail, the full
// metadata block and a scrollable description. The popup is a single shared
// element in document.body (so it anchors to the viewport regardless of
// ancestor transforms used by the virtual scroll). Leaving the card or
...
function getHoverPreviewEl() {
    ...
    // Any scroll or resize while the popup is visible invalidates its
    // card-anchored position, so dismiss rather than track.
...
function showHoverPreviewFor(card) {
    ...
    // Netflix/Apple-TV-style in-place zoom: anchor the popup to the card's
    // bounding rect, then scale it up from card-size to a larger size. The
    // transform-origin is chosen based on where the card sits in the viewport
    // so an edge-of-row card grows inward instead of being clipped.
    ...
    if (panel) {
        ...
        // When the pointer leaves the panel (and isn't heading back onto a
        // video card), dismiss. Re-attached per render because innerHTML
        // replaces the panel node each time.
    ...
    // Force a reflow so the starting transform/opacity is committed before
    // we flip `is-visible` and trigger the transition on the first hover.
    ...
    loadVideoDescription(videoId)
        .then((desc) => {
            // Guard: the user may have hovered another card before this resolved
...
function handleCardLeave(e) {
    ...
    // If the pointer moved into the popup panel, keep it open — the panel's
    // own mouseleave handler will dismiss when the pointer leaves it as well.
